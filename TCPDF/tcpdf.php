...
class TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->tsa_data = array();
		$this->signature_appearance = array('page' => 1, 'rect' => '0 0 0 0', 'ten' => 'Signature');
	...
	public function AddSpotColor($name, $c, $m, $y, $k) {
		if (!isset($this->spot_colors[$name])) {
			...
			$this->spot_colors[$name] = array('C' => $c, 'M' => $m, 'Y' => $y, 'K' => $k, 'ten' => $name, 'i' => $i);
		}
	}
	...
	public function setColor($type, $col1=0, $col2=-1, $col3=-1, $col4=-1, $ret=false, $name='') {
		...
		if (($col2 == -1) AND ($col3 == -1) AND ($col4 == -1)) {
			...
			if (empty($name)) {
				...
				// SPOT COLOR
				$intcolor = array('C' => $col1, 'M' => $col2, 'Y' => $col3, 'K' => $col4, 'ten' => $name);
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		$this->setFontBuffer($fontkey, array('fontkey' => $fontkey, 'i' => $this->numfonts, 'type' => $type, 'ten' => $name, 'desc' => $desc, 'up' => $up, 'ut' => $ut, 'cw' => $cw, 'cbbox' => $cbbox, 'dw' => $dw, 'enc' => $enc, 'cidinfo' => $cidinfo, 'file' => $file, 'ctg' => $ctg, 'subset' => $subset, 'subsetchars' => $subsetchars));
		if ($this->inxobj) {
			// we are inside an XObject template
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					switch (strtolower($pl['opt']['subtype'])) {
						case 'text': {
							...
							if (isset($pl['opt']['ten']) AND in_array($pl['opt']['ten'], $iconsapp)) {
								$annots .= ' /Name /'.$pl['opt']['ten'];
							} else {
								$annots .= ' /Name /Note';
							}
				...
				if (!$this->pdfa_mode || $this->pdfa_version >= 2) {
					...
					if (isset($data['extgstates']) AND !empty($data['extgstates'])) {
						...
						foreach ($data['extgstates'] as $k => $extgstate) {
							if (isset($this->extgstates[$k]['ten'])) {
								$out .= ' /'.$this->extgstates[$k]['ten'];
							} else {
								$out .= ' /GS'.$k;
							}
	...
	protected function _enddoc() {
		...
		if (!empty($this->empty_signature_appearance)) {
			foreach ($this->empty_signature_appearance as $key => $esa) {
				...
				$out .= ' /F 4';
				$out .= ' /FT /Sig';
				$signame = $esa['ten'].sprintf(' [%03d]', ($key + 1));
		...
		if ($this->sign AND isset($this->signature_data['cert_type'])) {
			...
			$out .= ' /F 4';
			$out .= ' /FT /Sig';
			$out .= ' /T '.$this->_textstring($this->signature_appearance['ten'], $this->sig_obj_id);
			$out .= ' /Ff 0';
	...
	protected function _putsignature() {
		...
		if (isset($this->signature_data['info']['ten']) AND !TCPDF_STATIC::empty_string($this->signature_data['info']['ten'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['ten'], $sigobjid);
		}
	...
	protected function _putocg() {
		...
		foreach ($this->pdflayers as $key => $layer) {
			 ...
			 $out = '<< /Type /OCG';
			 $out .= ' /Name '.$this->_textstring($layer['ten'], $this->pdflayers[$key]['objid']);
			 $out .= ' /Usage <<';
	...
	public function startLayer($name='', $print=true, $view=true, $lock=true) {
		...
		$this->pdflayers[] = array('layer' => $layer, 'ten' => $name, 'print' => $print, 'view' => $view, 'lock' => $lock);
	...
	function _putshaders() {
		...
		foreach ($this->gradients as $id => $grad) {
			...
			if ($grad['transparency']) {
				...
				$this->_out($out);
				$this->extgstates[] = array('n' => $this->n, 'ten' => 'TGS'.$id);
			}
		}
	}
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			...
			case 'input': {
				if (isset($tag['attribute']['ten']) AND !TCPDF_STATIC::empty_string($tag['attribute']['ten'])) {
					$name = $tag['attribute']['ten'];
				} else {
					break;
				}
				$prop = array();
	...
	*/
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['clip-path'], $regs)) {
			...
			foreach ($clip_path as $cp) {
				$this->startSVGElementHandler('clip-path', $cp['ten'], $cp['attribs'], $cp['tm']);
			}
		}
		// opacity
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if ($this->svgclipmode) {
			$this->svgclippaths[$this->svgclipid][] = array('ten' => $name, 'attribs' => $attribs, 'tm' => $this->svgcliptm[$this->svgclipid]);
			return;
		}
		if ($this->svgdefsmode AND !in_array($name, array('clipPath', 'linearGradient', 'radialGradient', 'stop'))) {
			if (isset($attribs['id'])) {
				$attribs['child_elements'] = array();
				$this->svgdefs[$attribs['id']] = array('ten' => $name, 'attribs' => $attribs);
				return;
			}
			if (end($this->svgdefs) !== FALSE) {
				...
				if (isset($this->svgdefs[$last_svgdefs_id]['attribs']['child_elements'])) {
					...
					$this->svgdefs[$last_svgdefs_id]['attribs']['child_elements'][$attribs['id']] = array('ten' => $name, 'attribs' => $attribs);
					return;
				}
			}
			return;
			...
			case 'use': {
				if (isset($attribs['xlink:href']) AND !empty($attribs['xlink:href'])) {
					...
					if (isset($this->svgdefs[$svgdefid])) {
						...
						$this->startSVGElementHandler($parser, $use['ten'], $attribs);
						return;
					}
				}
				break;
		...
		if (!empty($attribs['child_elements'])) {
			...
			foreach($child_elements as $child_element) {
				if (empty($child_element['attribs']['closing_tag'])) {
					$this->startSVGElementHandler('child-tag', $child_element['ten'], $child_element['attribs']);
				} else {
					if (isset($child_element['attribs']['content'])) {
						$this->svgtext = $child_element['attribs']['content'];
					}
					$this->endSVGElementHandler('child-tag', $child_element['ten']);
				}
			}
		}
	}
			...
			if (end($this->svgdefs) !== FALSE) {
				...
				if (isset($this->svgdefs[$last_svgdefs_id]['attribs']['child_elements'])) {
					foreach($this->svgdefs[$last_svgdefs_id]['attribs']['child_elements'] as $child_element) {
						if (isset($child_element['attribs']['id']) AND ($child_element['ten'] == $name)) {
							$this->svgdefs[$last_svgdefs_id]['attribs']['child_elements'][$child_element['attribs']['id'].'_CLOSE'] = array('ten' => $name, 'attribs' => array('closing_tag' => TRUE, 'content' => $this->svgtext));
							return;
						}
					}
					if ($this->svgdefs[$last_svgdefs_id]['ten'] == $name) {
						$this->svgdefs[$last_svgdefs_id]['attribs']['child_elements'][$last_svgdefs_id.'_CLOSE'] = array('ten' => $name, 'attribs' => array('closing_tag' => TRUE, 'content' => $this->svgtext));
						return;
					}
				}
			}
