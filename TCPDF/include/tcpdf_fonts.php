...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			$fmetric['ten'] = preg_replace('/[^a-zA-Z0-9_\-]/', '', $matches[1]);
			...
			for ($i = 0; $i < $numNameRecords; ++$i) {
				...
				if ($nameID == 6) {
					...
					$offset += 2;
					$offset = ($table['ten']['offset'] + $stringStorageOffset + $stringOffset);
					$fmetric['ten'] = substr($font, $offset, $stringLength);
					$fmetric['ten'] = preg_replace('/[^a-zA-Z0-9_\-]/', '', $fmetric['ten']);
					break;
				} else {
					$offset += 4; // skip String length, String offset
				}
			}
			if (empty($fmetric['ten'])) {
				$fmetric['ten'] = $font_name;
			}
			// ---------- get post data ----------
